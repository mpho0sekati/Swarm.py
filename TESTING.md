...
## 5. Testing the "Build" Feature
You can now use the swarm to build entire projects from the CLI.

```bash
make build PROMPT="Build a sentiment analysis tool using FastAPI" DIR="sentiment_app"
```

**Verification:**
- Check if the `sentiment_app/` directory is created.
- Verify that it contains valid Python code generated by the `Coder` agent.
- Review the `swarm_report.md` for the AI strategist's design.

---

## 6. Verifying the "Brain" (Learning)
